...
type BuildingType = "hq" | "refinery" | "barracks" | "foundry" | "intelligence" | "hangar" | "turret" | "exchange";
type ProductionBuilding = "hq" | "barracks" | "foundry" | "intelligence" | "hangar";
type BuildMode = "build-refinery" | "build-barracks" | "build-foundry" | "build-intelligence" | "build-hangar" | "build-turret" | "build-exchange";
type Building = {
  id: number;
  team: "player" | "enemy";
  type: BuildingType;
...
type Game = {
  ...
  camera: P;
  zoom: number;
  mode: "select" | "move" | "move-engage" | "move-hq" | "repair" | "set-patrol-a" | "set-patrol-b" | BuildMode | "set-rally";
  message: string;
  over: "" | "won" | "lost";
  nextId: number;
  waveAt: number;
  ...
  upgradeProduction?: { type: UpgradeKind; level: number; elapsed: number; duration: number };
  enemyUpgrades?: UpgradeLevels;
  enemyUpgradeProduction?: { type: UpgradeKind; level: number; elapsed: number; duration: number };
  /** Hidden AI economy posture chosen once per match. */
...
const buildingRenderSize = {
  ...
  barracks: { w: 116, h: 96 },
  foundry: { w: 138, h: 112 },
  intelligence: { w: 112, h: 104 },
  hangar: { w: 142, h: 104 },
  turret: { w: 88, h: 86 },
  exchange: { w: 104, h: 88 },
} as const;
const INTEL_RELAY_RENDER_SIZE = 130;
const buildingHealth = { hq: 900, refinery: 440, barracks: 520, foundry: 680, intelligence: 460, hangar: 560, turret: 360, exchange: 400 };
const turretStats = { damage: 12, range: 210, rate: 0.68 };
const buildingBuildTime = { refinery: 6, barracks: 6, foundry: 18, intelligence: 16, hangar: 20, turret: 15, exchange: 28 };
...
const MAX_QUEUE = 6;
const BUILD_COST = { refinery: 260, barracks: 360, foundry: 500, intelligence: 420, hangar: 480, turret: 240, exchange: 480 } as const;
function buildTypeFromMode(mode: Game["mode"]): Exclude<BuildingType, "hq"> | null {
  const table: Partial<Record<Game["mode"], Exclude<BuildingType, "hq">>> = {
...
const EXCHANGE_INCOME_RATE = 1.5;
const EXCHANGE_CIPHER_BOOST = 0.75;
const EXCHANGE_CIPHER_RADIUS = 260;
const EXCHANGE_MAX = 2;
const MAX_UPGRADE_LEVEL = 3;
const UPGRADE_COSTS = [0, 100, 180, 280] as const;
const UPGRADE_DURATIONS = [0, 25, 40, 60] as const;
const upgradeName = (type: UpgradeKind) =>
  type === "fireControl" ? "FIRE CONTROL" : type === "reinforcedFrames" ? "REINFORCED FRAMES" : "TACTICAL INTELLIGENCE";
const upgradeEffect = (type: UpgradeKind, level: number) =>
  type === "fireControl" ? `+${level * 8}% FIRE RATE`
    : type === "reinforcedFrames" ? `+${level * 10}% UNIT HP`
      : `+${level * 6}% RANGE + SENSORS`;
...
function emptyUpgrades(): UpgradeLevels {
  return { fireControl: 0, reinforcedFrames: 0, tacticalIntelligence: 0 };
}
function teamUpgrades(g: Game, team: Unit["team"]): UpgradeLevels {
  return team === "player" ? (g.upgrades || emptyUpgrades()) : (g.enemyUpgrades || emptyUpgrades());
}
function upgradedCombatRange(g: Game, unit: Unit) {
  return unitCombatRange(unit) * (1 + teamUpgrades(g, unit.team).tacticalIntelligence * .06);
}
function upgradedAttackRate(g: Game, unit: Unit) {
  return stats[unit.type].rate * (1 - teamUpgrades(g, unit.team).fireControl * .08);
}
function upgradedUnitHealth(g: Game, team: Unit["team"], type: Unit["type"]) {
  const doctrine = type === "tank" && teamDoctrine(g, team) === "armor" ? 1.18 : 1;
  return Math.round(unitHealth[type] * doctrine * (1 + teamUpgrades(g, team).reinforcedFrames * .1));
}
...
function formationTravelSpeed(g: Game, units: Unit[]) {
  if (units.length < 2 || !units.every(isCombatUnit)) return undefined;
  const slowest = Math.min(...units.map((unit) =>
    stats[unit.type].speed * (teamDoctrine(g, unit.team) === "air" && unit.type === "drone" ? 1.15 : 1)));
  // Coordinated formations get a modest cohesion bonus while still moving at
  // one shared pace, so fast escorts do not abandon armor on the approach.
  return slowest * (units.length >= 3 ? 1.1 : 1.06);
}
function exchangeCountForTeam(g: Game, team: Unit["team"]) {
  return g.buildings.filter((building) => building.team === team && building.type === "exchange").length;
}
function exchangeIncome(g: Game, exchange: Building) {
  if (exchange.type !== "exchange" || !buildingOperational(exchange)) return 0;
  const boosted = g.units.some((unit) =>
    unit.team === exchange.team && unit.type === "cipher" && unit.cipherMode === "deployed" &&
    g.time - (unit.lastCombatAt || -999) >= CIPHER_COMBAT_LOCKOUT &&
    Math.hypot(unit.x - exchange.x, unit.y - exchange.y) <= EXCHANGE_CIPHER_RADIUS);
  return EXCHANGE_INCOME_RATE + (boosted ? EXCHANGE_CIPHER_BOOST : 0);
}
...
function normalizeUnits(units: Unit[]): Unit[] {
  return units.map((raw) => {
    ...
    // Preserve researched/doctrine health from saves; rankMax is only the
    // fallback for older saves that did not persist a valid maximum.
    const max = Number.isFinite(raw.max) && raw.max > 0 ? raw.max : rankMax;
...
function initial(options: { fogEnabled?: boolean } = {}): Game {
  ...
  return {
    ...
    upgradeProduction: undefined,
    enemyUpgrades: emptyUpgrades(),
    enemyUpgradeProduction: undefined,
    enemyStrategy: (["rush", "balanced", "tech"] as const)[Math.floor(Math.random() * 3)],
...
function guestPerspective(authoritative: Game, local: Game | null, firstSnapshot: boolean): Game {
  ...
  [view.upgrades, view.enemyUpgrades] = [view.enemyUpgrades, view.upgrades];
  [view.upgradeProduction, view.enemyUpgradeProduction] = [view.enemyUpgradeProduction, view.upgradeProduction];
  ...
  revealFog(view);
  const enemyIntelligence = view.buildings.find((building) => building.team === "enemy" && building.type === "intelligence");
  const enemyTechVisible = Boolean(enemyIntelligence && isVisible(view, enemyIntelligence, buildingStats.intelligence.r));
  view.scoutedEnemyDoctrine = enemyTechVisible ? (view.enemyDoctrine || "none") : local?.scoutedEnemyDoctrine;
  ...
  return {
    ...
    constructionDuration:
      b.progress !== undefined
        ? Math.max(
            1,
            Number(b.constructionDuration) || buildingBuildTime[b.type],
          )
        : undefined,
    constructionStarted:
  ...
  const repaired: Game = {
    ...
    enemyTradeNetworkProduction:
      parsed.enemyTradeNetworkProduction && !parsed.enemyTradeNetwork
        ...
        : undefined,
    upgrades: {
      fireControl: Math.max(0, Math.min(MAX_UPGRADE_LEVEL, Number(parsed.upgrades?.fireControl) || 0)),
      reinforcedFrames: Math.max(0, Math.min(MAX_UPGRADE_LEVEL, Number(parsed.upgrades?.reinforcedFrames) || 0)),
      tacticalIntelligence: Math.max(0, Math.min(MAX_UPGRADE_LEVEL, Number(parsed.upgrades?.tacticalIntelligence) || 0)),
    },
    upgradeProduction:
      parsed.upgradeProduction && ["fireControl", "reinforcedFrames", "tacticalIntelligence"].includes(parsed.upgradeProduction.type)
        ? {
            type: parsed.upgradeProduction.type,
            level: Math.max(1, Math.min(MAX_UPGRADE_LEVEL, Number(parsed.upgradeProduction.level) || 1)),
            elapsed: Math.max(0, Number(parsed.upgradeProduction.elapsed) || 0),
            duration: Math.max(1, Number(parsed.upgradeProduction.duration) || 25),
          }
        : undefined,
    enemyUpgrades: {
      fireControl: Math.max(0, Math.min(MAX_UPGRADE_LEVEL, Number(parsed.enemyUpgrades?.fireControl) || 0)),
      reinforcedFrames: Math.max(0, Math.min(MAX_UPGRADE_LEVEL, Number(parsed.enemyUpgrades?.reinforcedFrames) || 0)),
      tacticalIntelligence: Math.max(0, Math.min(MAX_UPGRADE_LEVEL, Number(parsed.enemyUpgrades?.tacticalIntelligence) || 0)),
    },
    enemyUpgradeProduction:
      parsed.enemyUpgradeProduction && ["fireControl", "reinforcedFrames", "tacticalIntelligence"].includes(parsed.enemyUpgradeProduction.type)
        ? {
            type: parsed.enemyUpgradeProduction.type,
            level: Math.max(1, Math.min(MAX_UPGRADE_LEVEL, Number(parsed.enemyUpgradeProduction.level) || 1)),
            elapsed: Math.max(0, Number(parsed.enemyUpgradeProduction.elapsed) || 0),
            duration: Math.max(1, Number(parsed.enemyUpgradeProduction.duration) || 25),
          }
        : undefined,
    enemyStrategy: parsed.enemyStrategy === "rush" || parsed.enemyStrategy === "tech" ? parsed.enemyStrategy : "balanced",
...
export default function Home() {
  const canvas = useRef<HTMLCanvasElement>(null),
    ...
    art = useRef<{
      ...
      foundry?: HTMLImageElement;
      intelligence?: HTMLImageElement;
      hangar?: HTMLImageElement;
  ...
  const [ui, setUi] = useState({
    ...
    over: "",
    production: null as (Production & { building: ProductionBuilding }) | null,
    productionBuilding: null as ProductionBuilding | null,
    ...
    upgrades: emptyUpgrades(),
    upgradeProduction: null as { type: UpgradeKind; level: number; elapsed: number; duration: number } | null,
    exchangeCount: 0,
    exchangeIncome: 0,
    cipherCount: 0,
  ...
  useEffect(() => {
    let active = true;
    const load = (
      key: "terrain" | "units" | "workerDirections" | "workerWalk" | "workerWalkC" | "trooperDirections" | "trooperWalk" | "trooperWalkC" | "tankDirections" | "droneDirections" | "droneMove" | "cipherDirections" | "cipherDeployed" | "workerMine" | "turretDirections" | "buildings" | "foundry" | "intelligence" | "hangar" | "crystal" | "tacticalPlateau" | "commandCrawler" | "intelRelay",
      src: string,
    ) => {
    ...
    load("foundry", "/game-art/frontier-armor-foundry-v1.png");
    load("intelligence", "/game-art/frontier-intelligence-center-v1.png");
    load("hangar", "/game-art/frontier-drone-hangar-v1.png");
  ...
  const sync = () => {
    ...
    const hasOperational = (type: BuildingType) => g.buildings.some(
      (building) => building.team === "player" && building.type === type && buildingOperational(building),
    );
    setUi({
      ...
      production: selectedBuilding?.production
        ? {
            ...
            building: selectedBuilding.type as ProductionBuilding,
          }
        : null,
      productionCooldown: selectedBuilding?.cooldown || 0,
      productionBuilding:
        selectedBuilding &&
        ["hq", "barracks", "foundry", "intelligence", "hangar"].includes(selectedBuilding.type) &&
        buildingOperational(selectedBuilding)
          ? (selectedBuilding.type as ProductionBuilding)
          : null,
      ...
      hasFoundry: hasOperational("foundry"),
      hasIntelligence: hasOperational("intelligence"),
      hasHangar: hasOperational("hangar"),
      ...
      upgrades: { ...(g.upgrades || emptyUpgrades()) },
      upgradeProduction: g.upgradeProduction || null,
      exchangeCount: exchangeCountForTeam(g, "player"),
      exchangeIncome: g.buildings.filter((building) => building.team === "player" && building.type === "exchange").reduce((sum, building) => sum + exchangeIncome(g, building), 0),
  ...
  function applyRemotePoint(payload: Record<string, unknown>) {
    ...
    if (mode === "set-rally") {
      const building = g.buildings.find((b) =>
        b.team === "enemy" && selectedIds.includes(b.id) && ["hq", "barracks", "foundry", "intelligence", "hangar"].includes(b.type) && buildingOperational(b));
    ...
    if (mode.startsWith("build-")) {
      const type = buildTypeFromMode(mode);
      if (!type) return;
      const enemyHas = (required: BuildingType) => g.buildings.some((building) => building.team === "enemy" && building.type === required && buildingOperational(building));
      if ((type === "foundry" || type === "intelligence") && !enemyHas("barracks")) return;
      if (type === "hangar" && !enemyHas("intelligence")) return;
      if (type === "exchange" && (!g.enemyTradeNetwork || exchangeCountForTeam(g, "enemy") >= EXCHANGE_MAX)) return;
  ...
  function applyRemoteAction(payload: Record<string, unknown>) {
    ...
    if (name === "fortify") {
      if (!building || building.type !== "hq" || !buildingOperational(building) || g.enemyFortified || g.enemyFortifyProduction || g.enemyIntel < FORTIFY_INTEL_COST || building.production) return;
    ...
    if (name === "doctrine-air" || name === "doctrine-armor") {
      ...
      if (!building || building.type !== "intelligence" || !buildingOperational(building) || g.enemyDoctrine || g.enemyDoctrineProduction || g.enemyTradeNetworkProduction || g.enemyUpgradeProduction || g.enemyIntel < DOCTRINE_INTEL_COST || building.production) return;
    ...
    if (name === "trade-network") {
      if (!building || building.type !== "intelligence" || !buildingOperational(building) || g.enemyTradeNetwork || g.enemyTradeNetworkProduction || g.enemyDoctrineProduction || g.enemyUpgradeProduction || g.enemyIntel < TRADE_NETWORK_INTEL_COST || building.production) return;
    ...
    if (name.startsWith("upgrade-")) {
      const type = name.slice(8) as UpgradeKind;
      if (!["fireControl", "reinforcedFrames", "tacticalIntelligence"].includes(type)) return;
      const upgrades = g.enemyUpgrades ||= emptyUpgrades();
      const level = upgrades[type] + 1;
      const cost = UPGRADE_COSTS[level as 1 | 2 | 3];
      if (!building || building.type !== "intelligence" || !buildingOperational(building) || level > MAX_UPGRADE_LEVEL || g.enemyUpgradeProduction || g.enemyDoctrineProduction || g.enemyTradeNetworkProduction || building.production || g.enemyIntel < cost) return;
      g.enemyIntel -= cost;
      g.enemyUpgradeProduction = { type, level, elapsed: 0, duration: UPGRADE_DURATIONS[level as 1 | 2 | 3] };
      return;
    }
    if (name === "retreat") {
    ...
    if (name === "pack-hq" || name === "deploy-hq") {
      ...
      if (!hq || hq.production || g.enemyFortifyProduction || hq.relocation) return;
    ...
    const wanted = productionSource(type);
    building = g.buildings.find((b) =>
      b.team === "enemy" && selectedIds.includes(b.id) && b.type === wanted && buildingOperational(b));
    if (!building || (type === "worker" && g.enemyFortifyProduction) || (type === "cipher" && (g.enemyDoctrineProduction || g.enemyTradeNetworkProduction || g.enemyUpgradeProduction))) return;
  ...
  function guestAction(name: string) {
    ...
    if (name === "rally") {
      const building = g.buildings.find((b) => b.team === "player" && g.selected.includes(b.id) && ["hq", "barracks", "foundry", "intelligence", "hangar"].includes(b.type));
      if (!building || !buildingOperational(building)) {
        g.message = "Select a completed production building first.";
      } else {
        g.mode = "set-rally";
  ...
  const command = (wx: number, wy: number) => {
    ...
    if (g.mode === "set-rally") {
      const b = g.buildings.find(
        (x) =>
          ...
          ["hq", "barracks", "foundry", "intelligence", "hangar"].includes(x.type) &&
    ...
    if (g.mode.startsWith("build")) {
      const type = buildTypeFromMode(g.mode);
      if (!type) return;
      ...
      if (type === "exchange" && (!g.tradeNetwork || exchangeCountForTeam(g, "player") >= EXCHANGE_MAX)) {
        g.message = !g.tradeNetwork ? "Research Trade Network before constructing an Exchange." : `Exchange limit reached (${EXCHANGE_MAX}).`;
        sync();
        return;
      }
      const blocked =
    ...
    const formationSpeed = formationTravelSpeed(g, ours);
    if (attacking)
  ...
  const action = (name: string) => {
      ...
      g.message =
        name === "build-refinery"
          ...
          : name === "build-barracks"
            ...
            : name === "build-exchange"
              ? "EXCHANGE PLACEMENT: tap a protected location for long-term credit income."
    ...
    if (name === "pack-hq" || name === "deploy-hq" || name === "move-hq") {
      ...
      if (!b || b.type !== "hq") {
        ...
        else if (b.production || g.fortifyProduction) g.message = "HQ is busy — finish Worker production or Fortify Base before packing.";
        else {
    ...
    if (name === "rally") {
      if (!b || !(["hq", "barracks", "foundry", "intelligence", "hangar"] as string[]).includes(b.type) || !buildingOperational(b)) {
        g.message =
          "Select a completed production building first, then set its rally point.";
        sync();
        return;
      }
      g.mode = "set-rally";
    ...
    if (name === "sell") {
      if (!b || b.type === "hq") {
        g.message =
          "Select a non-HQ structure first, then tap Sell Selected.";
        sync();
        return;
      }
    ...
    if (name.startsWith("upgrade-")) {
      ...
      if (!b || b.type !== "intelligence" || !buildingOperational(b)) {
        ...
        g.intel -= UPGRADE_COSTS[level as 1 | 2 | 3];
        g.upgradeProduction = { type, level, elapsed: 0, duration: UPGRADE_DURATIONS[level as 1 | 2 | 3] };
        g.matchStats.meaningfulActions++;
        g.message = `${upgradeName(type)} level ${level} research started.`;
      }
      sync();
      return;
    }
    ...
    if (type === "worker" && g.fortifyProduction) {
      g.message = `HQ is busy with Fortify Base — ${unitName(type)} production is paused.`;
      sync();
      return;
    }
  ...
  function update(dt: number) {
    ...
    revealFog(g);
    const enemyIntelligence = g.buildings.find((building) => building.team === "enemy" && building.type === "intelligence");
    if (enemyIntelligence && isVisible(g, enemyIntelligence, buildingStats.intelligence.r)) {
    ...
    for (const team of ["player", "enemy"] as const) {
      ...
      if (!production) continue;
      if (!g.buildings.some((building) => building.team === team && building.type === "intelligence" && buildingOperational(building))) continue;
      production.elapsed = Math.min(production.duration, production.elapsed + dt);
      if (production.elapsed >= production.duration) {
        if (team === "player") {
    ...
    for (const team of ["player", "enemy"] as const) {
      const production = team === "player" ? g.upgradeProduction : g.enemyUpgradeProduction;
      if (!production) continue;
      if (!g.buildings.some((building) => building.team === team && building.type === "intelligence" && buildingOperational(building))) continue;
      production.elapsed = Math.min(production.duration, production.elapsed + dt);
      if (production.elapsed < production.duration) continue;
      const upgrades = team === "player" ? (g.upgrades ||= emptyUpgrades()) : (g.enemyUpgrades ||= emptyUpgrades());
      const previousLevel = upgrades[production.type];
      upgrades[production.type] = production.level;
      if (production.type === "reinforcedFrames") {
        const previousMultiplier = 1 + previousLevel * .1;
        const nextMultiplier = 1 + production.level * .1;
        for (const unit of g.units.filter((candidate) => candidate.team === team)) {
          const gained = Math.round(unit.max * (nextMultiplier / previousMultiplier - 1));
          unit.max += gained;
          unit.hp += gained;
        }
      }
      if (team === "player") {
        g.upgradeProduction = undefined;
        g.message = `${upgradeName(production.type)} level ${production.level} complete.`;
      } else {
        g.enemyUpgradeProduction = undefined;
      }
      sync();
    }
    for (const exchange of g.buildings.filter((building) => building.type === "exchange")) {
      const income = exchangeIncome(g, exchange) * dt;
      if (exchange.team === "player") g.credits += income;
      else g.enemyCredits += income;
    }
    for (const b of g.buildings) {
      ...
      const duration = b.constructionDuration || buildingBuildTime[b.type];
    ...
    for (const b of g.buildings)
      if (
        ...
        !(b.type === "hq" && ((b.team === "player" && g.fortifyProduction) || (b.team === "enemy" && g.enemyFortifyProduction))) &&
        !(b.type === "intelligence" && ((b.team === "player" && (g.doctrineProduction || g.tradeNetworkProduction || g.upgradeProduction)) || (b.team === "enemy" && (g.enemyDoctrineProduction || g.enemyTradeNetworkProduction || g.enemyUpgradeProduction))))
      ) {
        ...
        if (b.production.elapsed >= b.production.duration) {
          ...
          const id = g.nextId++;
          const deployedHealth = upgradedUnitHealth(g, b.team, type);
      ...
      if (target) {
        ...
        if (d > range) {
          ...
          if (attackTimers.current[u.id] <= 0) {
            ...
            const commandRateBonus = multiplayerRole.current === "solo" && u.team === "player"
              ? 1 - Math.max(0, Math.min(FIRE_CONTROL_MAX_RANK, commandProfile.fireControlRank || 0)) * .02
              : 1;
            attackTimers.current[u.id] = upgradedAttackRate(g, u) * commandRateBonus;
          }
        }
      } else if (u.target) {
  ...
  function runAI(g: Game) {
    ...
    const strategy = g.enemyStrategy || "balanced";
    ...
    const enemyBuildings = g.buildings.filter((b) => b.team === "enemy"),
      ...
      foundry = enemyBuildings.find((b) => b.type === "foundry"),
      intelligence = enemyBuildings.find((b) => b.type === "intelligence"),
      hangar = enemyBuildings.find((b) => b.type === "hangar"),
      workers = g.units.filter(
        (u) => u.team === "enemy" && u.type === "worker",
      ),
      army = g.units.filter((u) => u.team === "enemy" && isCombatUnit(u));
    if (!hq) return;
    const queueAiStructure = (type: Exclude<BuildingType, "hq">, x: number, y: number) => {
      if (g.enemyAlloy < BUILD_COST[type]) return false;
      const r = buildingStats[type].r;
      const clear = (site: P) => !g.buildings.some((candidate) => Math.hypot(candidate.x - site.x, candidate.y - site.y) < buildingStats[candidate.type].r + r + 18) &&
        !g.crystals.some((crystal) => crystal.amount > 0 && Math.hypot(crystal.x - site.x, crystal.y - site.y) < r + 42) &&
        !TERRAIN_RIDGES.some((ridge) => Math.hypot(ridge.x - site.x, ridge.y - site.y) < ridge.r + r + 18) &&
        !(g.objectives || []).some((objective) => Math.hypot(objective.x - site.x, objective.y - site.y) < RELAY_RESOURCE_CLEARANCE);
      const site = [{ x, y }, { x: x - 120, y }, { x, y: y + 120 }, { x: x + 120, y }, { x, y: y - 120 }]
        .map((candidate) => ({ x: Math.max(r + 30, Math.min(W - r - 30, candidate.x)), y: Math.max(r + 30, Math.min(H - r - 30, candidate.y)) }))
        .find(clear);
      if (!site) return false;
      g.enemyAlloy -= BUILD_COST[type];
      const building: Building = { id: g.nextId++, team: "enemy", type, ...site, hp: 1, max: buildingHealth[type], progress: 0, constructionDuration: buildingBuildTime[type], constructionStarted: false };
    ...
    const intelligenceOnline = Boolean(intelligence && buildingOperational(intelligence));
    if (intelligenceOnline && !g.enemyDoctrine && !g.enemyDoctrineProduction && !intelligence!.production && g.enemyIntel >= DOCTRINE_INTEL_COST) {
    ...
    if (intelligenceOnline && !g.enemyTradeNetwork && !g.enemyTradeNetworkProduction && !g.enemyDoctrineProduction && !g.enemyUpgradeProduction && !intelligence!.production && g.enemyIntel >= TRADE_NETWORK_INTEL_COST && (strategy === "tech" || g.time >= 300)) {
      g.enemyIntel -= TRADE_NETWORK_INTEL_COST;
      g.enemyTradeNetworkProduction = { elapsed: 0, duration: TRADE_NETWORK_DURATION };
      nextAction();
      return;
    }
    if (intelligenceOnline && !g.enemyUpgradeProduction && !g.enemyDoctrineProduction && !g.enemyTradeNetworkProduction && !intelligence!.production) {
      const priorities: UpgradeKind[] = strategy === "rush"
        ? ["fireControl", "tacticalIntelligence", "reinforcedFrames"]
        : strategy === "tech"
          ? ["tacticalIntelligence", "reinforcedFrames", "fireControl"]
          : ["reinforcedFrames", "fireControl", "tacticalIntelligence"];
      const upgrades = g.enemyUpgrades ||= emptyUpgrades();
      const choice = priorities.find((type) => upgrades[type] < MAX_UPGRADE_LEVEL);
      if (choice) {
        const level = upgrades[choice] + 1;
        const cost = UPGRADE_COSTS[level as 1 | 2 | 3];
        if (g.enemyIntel >= cost && (strategy === "tech" || g.wave > 0 || army.length >= 8)) {
          g.enemyIntel -= cost;
          g.enemyUpgradeProduction = { type: choice, level, elapsed: 0, duration: UPGRADE_DURATIONS[level as 1 | 2 | 3] };
          nextAction();
          return;
        }
      }
    ...
    if (barracks && buildingOperational(barracks)) {
      const wantsIntelFirst = strategy === "tech";
      if (wantsIntelFirst && !intelligence && queueAiStructure("intelligence", hq.x - 185, hq.y - 30)) return;
      if (!foundry && queueAiStructure("foundry", hq.x + 165, hq.y + 140)) return;
      if (!intelligence && queueAiStructure("intelligence", hq.x - 185, hq.y - 30)) return;
    }
    if (intelligenceOnline && !hangar && (strategy !== "rush" || g.time >= 180) && queueAiStructure("hangar", hq.x + 185, hq.y - 125)) return;
    const enemyExchanges = g.buildings.filter((building) => building.team === "enemy" && building.type === "exchange");
    const exchangeGoal = strategy === "tech" ? 2 : strategy === "balanced" ? 1 : 0;
    if (g.enemyTradeNetwork && enemyExchanges.length < exchangeGoal && g.enemyAlloy >= BUILD_COST.exchange) {
      const offsets = [{ x: -165, y: -80 }, { x: 145, y: 115 }];
      const site = offsets[enemyExchanges.length] || offsets[0];
      const building: Building = {
        id: g.nextId++, team: "enemy", type: "exchange",
        x: Math.max(80, Math.min(W - 80, hq.x + site.x)),
        y: Math.max(80, Math.min(H - 80, hq.y + site.y)),
        hp: 1, max: buildingHealth.exchange, progress: 0,
        constructionDuration: buildingBuildTime.exchange, constructionStarted: false,
      };
      const blocked =
        g.buildings.some((candidate) => Math.hypot(candidate.x - building.x, candidate.y - building.y) < buildingStats[candidate.type].r + buildingStats.exchange.r + 18) ||
        g.crystals.some((crystal) => crystal.amount > 0 && Math.hypot(crystal.x - building.x, crystal.y - building.y) < buildingStats.exchange.r + 44) ||
        TERRAIN_RIDGES.some((ridge) => Math.hypot(ridge.x - building.x, ridge.y - building.y) < ridge.r + buildingStats.exchange.r + 18) ||
        (g.objectives || []).some((objective) => Math.hypot(objective.x - building.x, objective.y - building.y) < RELAY_RESOURCE_CLEARANCE);
      if (!blocked) {
        g.enemyAlloy -= BUILD_COST.exchange;
        g.buildings.push(building);
        const builder = [...workers].sort((a, b) => Math.hypot(a.x - building.x, a.y - building.y) - Math.hypot(b.x - building.x, b.y - building.y))[0];
        if (builder) queueWorkerConstruction(builder, building.id);
        nextAction();
    ...
    const deployedCiphers = g.units.filter((unit) => unit.team === "enemy" && unit.type === "cipher");
    const operationalExchange = enemyExchanges.find(buildingOperational);
    const mobileCipher = deployedCiphers.find((unit) => (unit.cipherMode || "mobile") === "mobile" && !unit.target);
    if (operationalExchange && mobileCipher) {
      if (Math.hypot(mobileCipher.x - operationalExchange.x, mobileCipher.y - operationalExchange.y) <= EXCHANGE_CIPHER_RADIUS * .65) {
        mobileCipher.cipherMode = "deploying";
        mobileCipher.cipherProgress = 0;
      } else {
        mobileCipher.target = { x: operationalExchange.x + 72, y: operationalExchange.y + 36 };
      }
      nextAction();
      return;
    }
    const workerGoal = Math.round((strategy === "tech" ? 4 : 3) * ai);
    if (workers.length < workerGoal && g.enemyCredits >= 150 && !hq.production && !(hq.cooldown || 0) && !g.enemyFortifyProduction) {
      g.enemyCredits -= 150;
      hq.production = {
        type: "worker",
        elapsed: 0,
    ...
    if (g.enemyTradeNetwork && operationalExchange && intelligenceOnline && deployedCiphers.length < (strategy === "tech" ? 2 : 1) && g.enemyCredits >= unitCost.cipher && !intelligence!.production && !(intelligence!.cooldown || 0) && !g.enemyDoctrineProduction && !g.enemyTradeNetworkProduction && !g.enemyUpgradeProduction) {
      g.enemyCredits -= unitCost.cipher;
      intelligence!.production = { type: "cipher", elapsed: 0, duration: productionDurationFor(g, "enemy", "cipher") };
      nextAction();
      return;
    }
    ...
    if (foundry && buildingOperational(foundry) && !foundry.production && !(foundry.cooldown || 0) && army.length >= Math.round(5 * ai) && g.enemyCredits >= unitCost.tank && g.wave > 0) {
      g.enemyCredits -= unitCost.tank;
      foundry.production = { type: "tank", elapsed: 0, duration: productionDurationFor(g, "enemy", "tank") };
      nextAction();
      return;
    }
    const enemyDrones = army.filter((unit) => unit.type === "drone").length;
    const enemyTroopers = army.filter((unit) => unit.type === "trooper").length;
    if (hangar && buildingOperational(hangar) && !hangar.production && !(hangar.cooldown || 0) && g.enemyCredits >= unitCost.drone && (playerArmor >= 2 || (enemyTroopers >= 3 && enemyDrones * 3 < enemyTroopers))) {
      g.enemyCredits -= unitCost.drone;
      hangar.production = { type: "drone", elapsed: 0, duration: productionDurationFor(g, "enemy", "drone") };
      nextAction();
      return;
    }
    ...
    const required = easiest
      ...
      : Math.min(16, Math.max(4, Math.round((6 + g.wave * 2 + (strategy === "tech" ? 3 : strategy === "rush" ? -1 : 0)) * ai)));
    const openingGrace = easiest && g.wave === 0 ? 240 : strategy === "tech" && g.wave === 0 ? 210 : 0;
    if (g.time >= Math.max(g.aiAttackAt, openingGrace) && ready.length >= required) {
      ...
      if (target) {
        const formation = formationDestinations(ready, target);
        const formationSpeed = formationTravelSpeed(g, ready);
        ready.forEach((u, i) => {
          u.enemy = target.id;
          u.target = formation.get(u.id) || { x: target.x + (i % 3) * 24, y: target.y + Math.floor(i / 3) * 24 };
          u.formationSpeed = formationSpeed;
          u.nav = undefined;
        });
      }
  ...
  function draw() {
    ...
    for (const b of g.buildings) {
      ...
      x.save();
      if (sel && (b.type === "hq" || b.type === "exchange") && buildingOperational(b)) {
        x.strokeStyle = b.type === "exchange" ? "rgba(246, 211, 102, .3)" : "rgba(87, 215, 192, .22)";
        x.lineWidth = 2;
        x.setLineDash([12, 10]);
        x.beginPath();
        x.arc(b.x, b.y, b.type === "exchange" ? EXCHANGE_CIPHER_RADIUS : SUPPLY_RADIUS, 0, Math.PI * 2);
        x.stroke();
        x.setLineDash([]);
      }
      x.translate(b.x, b.y);
      ...
      const specialBuilding = b.type === "foundry" ? art.current.foundry : b.type === "intelligence" ? art.current.intelligence : b.type === "hangar" ? art.current.hangar : undefined;
      if (underConstruction) {
        x.globalAlpha = b.constructionStarted ? Math.max(.2, b.progress * .78) : .14;
        x.filter = "grayscale(.55) brightness(1.25)";
      }
      if (buildingAtlas || turretAtlas || crawlerAtlas || specialBuilding) {
        if (sel) {
          x.shadowBlur = 0;
        ...
        if (crawlerAtlas) {
        ...
        } else if (specialBuilding) {
          x.drawImage(specialBuilding, -size.w / 2, -size.h / 2, size.w, size.h);
        } else if (turretAtlas) {
        ...
        } else if (buildingAtlas && !(["foundry", "intelligence", "hangar"] as string[]).includes(b.type)) {
          const frame = { hq: 0, refinery: 1, barracks: 2, turret: 3, exchange: 1 }[b.type];
          const sw = buildingAtlas.naturalWidth / 2;
          const sh = buildingAtlas.naturalHeight / 2;
          if (b.type === "exchange") x.filter = `${underConstruction ? "grayscale(.35) brightness(1.15) " : ""}hue-rotate(58deg) saturate(1.35)`;
          x.drawImage(
            buildingAtlas,
            (frame % 2) * sw,
          ...
          if (b.type === "exchange") x.filter = underConstruction ? "grayscale(.55) brightness(1.25)" : "none";
        }
      ...
      if (b.progress !== undefined && b.progress < 1) {
        ...
        const seconds = Math.max(
          0,
          Math.ceil((b.constructionDuration || buildingBuildTime[b.type]) * (1 - b.progress)),
        );
      ...
      if (b.type === "intelligence" && b.team === "player" && g.doctrineProduction) {
    ...
    for (const u of g.units) {
      ...
      if (sel && upgradedCombatRange(g, u) > 0) {
        x.save();
        x.strokeStyle = "rgba(246, 211, 102, .78)";
        ...
        x.shadowBlur = 5;
        x.beginPath();
        x.arc(u.x, u.y, upgradedCombatRange(g, u), 0, Math.PI * 2);
        x.fill();
        x.stroke();
        x.restore();
      }
      x.save();
      x.translate(u.x, u.y + (u.type === "drone" && !sentryMode ? (u.moving || (u.attackUntil || 0) > g.time ? Math.sin(g.time * 5 + u.id) * 1.4 - 7 : -7) : 0));
    ...
    if (p.drag && !matchMedia("(pointer: coarse)").matches) {
      ...
      if (hit && hitDistance < 55 && g.mode === "select") {
        ...
        if (unit && repeat) {
          ...
          building &&
          repeat &&
          ["hq", "barracks", "foundry", "intelligence", "hangar"].includes(building.type)
        ) {
  ...
  const activeTip = ui.selectedBuilding === "hq" && (ui.hqPacked || ui.hqRelocation)
    ...
    : ui.selectedWorkers > 0
      ...
      : ui.selectedCombat > 0
        ...
        : { key: "select-units", title: "FIELD TUTORIAL", text: "Select a Worker to construct, then select completed production buildings to train their assigned unit class." };
  ...
  const productionButton = (type: Unit["type"]) => {
    ...
    const queuedCount = (active ? 1 : 0) + (ui.production?.queue || []).filter((queuedType) => queuedType === type).length;
    ...
    const source = productionSource(type);
    const researchBusy = type === "cipher" && Boolean(ui.doctrineProduction || ui.tradeNetworkProduction || ui.upgradeProduction);
    const locked = ui.productionBuilding !== source || coolingDown || cipherLocked || researchBusy || (type === "worker" && Boolean(ui.fortifyProduction));
    const idle =
      type === "worker"
    ...
    return (
      ...
      >
        ...
        <span className={`command-art unit-${type}`} aria-hidden="true">
          ...
          {queuedCount > 0 && <b className="production-queue-badge">{queuedCount}</b>}
          {active && (
            <em className="production-progress-overlay" style={{ "--progress": `${(ui.production!.elapsed / ui.production!.duration) * 100}%` } as React.CSSProperties} />
          )}
        </span>
        <span>
          {unitName(type)}
      ...
      {homeOpen && (
        <section className="home-screen" aria-label="Frontier Command home screen">
          ...
          <div className="home-panel">
            ...
            {hasAutosave && (
              <button className="continue-match" onClick={game.current.over ? () => startNewMatch(game.current.fogEnabled) : continueMatch}>
                <b>{game.current.over ? "START NEW MATCH" : "CONTINUE MATCH"}</b>
                <small>{game.current.over ? "PREVIOUS OPERATION COMPLETE" : `${game.current.fogEnabled ? "TACTICAL FOG" : "OPEN INTEL"} · WAVE ${game.current.wave}`}</small>
              </button>
            )}
          ...
          {ui.selectedCombat > 1 && ui.selectedWorkers === 0 ? (
            <div className="formation-panel">
              <div className="formation-header">
                <div><small>FORMATION</small><b>{ui.selectedCombat} UNITS · COHESION +{ui.selectedCombat >= 3 ? 10 : 6}%</b></div>
              ...
              <div className="formation-orders" aria-label="Formation orders">
                ...
                <button onClick={() => action("move-engage")} aria-label="Move and engage" title="Move and engage">✦</button>
          ...
          <div className="command-context" aria-label="Current command menu">
            {(commandTab === "buildings" || commandTab === "tech") ? (
              <button className="context-back" onClick={() => setCommandTab("units")}>
                <i>‹</i>
                <span>{commandTab === "buildings" ? "CONSTRUCTION" : "INTEL RESEARCH"}<small>BACK TO ORDERS</small></span>
              </button>
            ) : ui.selectedUnits > 0 ? (
          ...
          <div className="actions" role="tabpanel">
            {ui.selectedWorkers > 0 && commandTab === "buildings" ? (
              <>
                ...
                <button disabled={!ui.hasBarracks || ui.alloy < BUILD_COST.foundry} className={`${ui.buildMode === "build-foundry" ? "placing " : ""}${!ui.hasBarracks ? "locked" : ui.alloy < BUILD_COST.foundry ? "unaffordable" : ""}`} aria-pressed={ui.buildMode === "build-foundry"} onClick={() => action("build-foundry")}>
                ...
                <button disabled={!ui.tradeNetwork || ui.exchangeCount >= EXCHANGE_MAX || ui.alloy < BUILD_COST.exchange} className={`${ui.buildMode === "build-exchange" ? "placing " : ""}${ui.alloy < BUILD_COST.exchange ? "unaffordable" : !ui.tradeNetwork || ui.exchangeCount >= EXCHANGE_MAX ? "locked" : ""}`} aria-pressed={ui.buildMode === "build-exchange"} onClick={() => action("build-exchange")}>
                  <i>¤</i><span className="command-art building-exchange" aria-hidden="true" /><span>TRADE EXCHANGE<small>{!ui.tradeNetwork ? "REQUIRES TRADE NETWORK" : ui.exchangeCount >= EXCHANGE_MAX ? `LIMIT ${EXCHANGE_MAX}` : ui.alloy < BUILD_COST.exchange ? `NEED ${BUILD_COST.exchange - ui.alloy} MORE ALLOY` : `${BUILD_COST.exchange} ALLOY · +90 CREDITS/MIN`}</small></span>
                </button>
              </>
            ) : ui.selectedUnits > 0 ? (
              <>
            ...
            ) : ui.selectedBuilding === "intelligence" && commandTab === "tech" ? (
              <>
                {(["air", "armor"] as Doctrine[]).map((doctrine) => {
                  ...
                  return (
                    <button key={doctrine} disabled={Boolean(ui.doctrine) || Boolean(ui.doctrineProduction) || Boolean(ui.tradeNetworkProduction) || Boolean(ui.upgradeProduction) || ui.intel < DOCTRINE_INTEL_COST} className={chosen || locked || Boolean(ui.tradeNetworkProduction) || Boolean(ui.upgradeProduction) ? "locked" : researching ? "placing" : ui.intel < DOCTRINE_INTEL_COST ? "unaffordable" : ""} onClick={() => action(`doctrine-${doctrine}`)} title={ui.intel < DOCTRINE_INTEL_COST ? `${DOCTRINE_INTEL_COST - ui.intel} more intel required` : undefined}>
                ...
                {(["fireControl", "reinforcedFrames", "tacticalIntelligence"] as UpgradeKind[]).map((type) => {
                  const level = ui.upgrades[type];
                  const nextLevel = level + 1;
                  const researching = ui.upgradeProduction?.type === type;
                  const maxed = level >= MAX_UPGRADE_LEVEL;
                  const cost = maxed ? 0 : UPGRADE_COSTS[nextLevel as 1 | 2 | 3];
                  const blocked = Boolean(ui.doctrineProduction || ui.tradeNetworkProduction || (ui.upgradeProduction && !researching));
                  return (
                    <button key={type} disabled={maxed || blocked || Boolean(ui.upgradeProduction) || ui.intel < cost} className={maxed || blocked ? "locked" : researching ? "placing" : ui.intel < cost ? "unaffordable" : ""} onClick={() => action(`upgrade-${type}`)}>
                      <i>{type === "fireControl" ? "⌁" : type === "reinforcedFrames" ? "⬡" : "◎"}</i>
                      <span>{upgradeName(type)} · L{level}<small>{maxed ? `${upgradeEffect(type, level)} · MAX` : researching ? `${Math.max(0, Math.ceil(ui.upgradeProduction!.duration - ui.upgradeProduction!.elapsed))}s REMAINING` : ui.intel < cost ? `NEED ${cost - ui.intel} MORE INTEL` : `${cost} INTEL · ${UPGRADE_DURATIONS[nextLevel as 1 | 2 | 3]}s · ${upgradeEffect(type, nextLevel)}`}</small></span>
                    </button>
                  );
                })}
              ...
              ui.hqPacked ? <>
                ...
                <button disabled={ui.fortified || Boolean(ui.fortifyProduction) || Boolean(ui.production) || ui.intel < FORTIFY_INTEL_COST} className={ui.fortified ? "locked" : ui.fortifyProduction ? "placing" : ui.intel < FORTIFY_INTEL_COST ? "unaffordable" : ""} onClick={() => action("fortify")}>
                  <kbd>F</kbd><i>⬡</i><span>{ui.fortified ? "BASE FORTIFIED" : ui.fortifyProduction ? "FORTIFYING BASE" : "FORTIFY BASE"}<small>{ui.fortified ? "+25% STRUCTURE HP ACTIVE" : ui.fortifyProduction ? `${Math.max(0, Math.ceil(ui.fortifyProduction.duration - ui.fortifyProduction.elapsed))}s REMAINING` : `${FORTIFY_INTEL_COST} INTEL · +25% STRUCTURE HP`}</small></span>
                </button>
              ...
              ui.productionBuilding === "barracks" ? <>
                {productionButton("trooper")}
                <button onClick={() => action("rally")}><kbd>G</kbd><i>⌖</i><span>SET WAYPOINT<small>COMBAT UNIT DEPLOYMENT</small></span></button>
                <button className="sell" onClick={() => action("sell")}><i>✕</i><span>SELL BARRACKS<small>50% · {Math.floor(BUILD_COST.barracks / 2)} ALLOY</small></span></button>
            ...
            ) : ui.selectedBuilding === "foundry" ? (
              ui.productionBuilding === "foundry" ? <>{productionButton("tank")}<button onClick={() => action("rally")}><kbd>G</kbd><i>⌖</i><span>SET WAYPOINT<small>ARMOR DEPLOYMENT</small></span></button><button className="sell" onClick={() => action("sell")}><i>✕</i><span>SELL FOUNDRY<small>50% · {Math.floor(BUILD_COST.foundry / 2)} ALLOY</small></span></button></> : null
              ...
              <div className="production-empty">
                ...
                <small>HQ: Workers · Barracks: Troopers · Foundry: Tanks · Intel Center: research · Hangar: Drones.</small>
              </div>
            )}
          </div>
          </>)}
